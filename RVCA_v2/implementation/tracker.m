...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            response_d{k1} = bsxfun(@times, conj(cf_f{k1}), xtf{k1}); %各通道响应
...
%             end
            ...
            for i = 1:params.F
                ...
                Prev_response_d{i} = circshift(Prev_response_d{i},shift_y,1);
                M_prev{i}= circshift(M_prev{i},shift_x,2); 
                Prev_response_d{i} = circshift(Prev_response_d{i},shift_x,2);
            end
...
%             lambda3_norm = admm_lambda_3/2 * norm(permute(beta_k,[2,3,1]) - permute(beta_0,[2,3,1]), 2)
    ...
    if(seq.frame == 1)
            % Compute convolution for each feature block in the Fourier domain
            % and the sum over all blocks.
            response_d{k1} = bsxfun(@times, conj(cf_f{k1}), xlf{k1}); % 每个通道响应，频域上
            scores_fs_feat{k1} = gather(sum(response_d{k1}, 3));
...
%                 M_prev{m} = M_prev{m-1};
            ...
            for m = params.F : -1 :2
                ...
                M_prev{m} = M_prev{m-1};
                Prev_response_d{m} = Prev_response_d{m-1};% 各通道响应
