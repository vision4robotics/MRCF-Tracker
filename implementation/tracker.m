...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            if params.clamp_position% Clamp the target position to be inside the image
    ...
    if seq.frame == 1
        ...
        shift_samp_pos = 2*pi * translation_vec ./(currentScaleFactor* img_sample_sz);
        xlf{1} = shift_sample(xtf{1}, shift_samp_pos, kx', ky');
    end
...
%                 beta_k = admm_lambda_3 * beta_0 ./ (admm_lambda * sum(sum(t .* t, 2), 1) + admm_lambda_3);
                if admm_lambda_3>0
                    S_h_x = h_f .* model_xf;
                    beta_k = (T * admm_lambda_3 * beta_0 + sum(sum( conj(S_h_x) .* (yf + M_train), 2), 1) )./ ( (1 + admm_lambda_2) * sum(sum( conj(S_h_x) .* (S_h_x), 2), 1) + T * admm_lambda_3);
                end            
                %   update L
                l_f = l_f + (mu * (q_f - h_f));
